...
class prediction:
    ...
    def predictionFromModel(self):
        ...
        try:
            ...
            try:
                with open('models/feature_columns.json', 'r') as feature_file:
                    feature_columns = json.load(feature_file)
            except FileNotFoundError as file_error:
                self.log_writer.log(self.file_object, 'Feature column mapping file missing. Error:: %s' % file_error)
                raise file_error
            ...
            extra_columns = [col for col in data.columns if col not in feature_columns]
            if extra_columns:
